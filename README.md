...
- **Fetches are sequential, not concurrent.** All three initial pulls
  (Upcoming Campouts, Roster, Uncompleted Requirements) run one after
  another rather than via `Promise.all()` -- TWH's server-side session
  state can race silently on concurrent requests and return empty
  results. An earlier version of this merge used `Promise.all()`;
  fixed to match every other tool in this repo.
- **Tab-switch width stability.** TroopWebHost's page chrome is old
  ASP.NET WebForms and likely lays its content area out with a
  table-based structure rather than plain block flow -- a table cell (or
  anything else that shrink-wraps to its content) resizes to fit
  whichever tab panel is currently visible, so if inactive panels are
  removed from layout with `display:none`, the Campouts table (typically
  much wider than the Patrols table or the All Scouts panel's single
  button) makes the whole page's width visibly jump on every tab switch.
  A first attempt fixed this by JS-measuring the natural combined width
  of all three panels once after data loaded and pinning it as a
  `min-width` -- this turned out not to be reliable against TWH's real
  page chrome (confirmed still jumping after pasting onto the live
  site), likely because the measurement's own shrink-to-fit pass is
  itself constrained by whatever width the not-yet-stabilized ancestor
  currently happens to have, a timing dependency that doesn't reliably
  resolve through however many legacy table layers sit above it.
  Replaced with a purely-CSS fix that has no timing dependency at all:
  the three tab panels are stacked into the same CSS Grid cell
  (`.rg-mode-stack` / `.rg-mode-panel`) and the inactive ones are hidden
  with `visibility:hidden` instead of `display:none` -- unlike
  `display:none`, `visibility:hidden` still occupies its layout space,
  so the stack's own box is always the union of all three panels' sizes,
  constant on every single render regardless of which is active, with
  nothing to keep in sync. The trade-off: the stack's height is always
  tall enough for the tallest tab, so a short tab (All Scouts) shows
  blank space below it rather than the page shrinking to fit -- a fixed,
  minor cost for guaranteed stability on any page chrome. Like the
  earlier attempt, this isn't something the jsdom/headless-Chromium
  harnesses in this repo can prove fixes the live-site symptom, since
  neither runs inside TWH's actual page chrome (a plain `<body>` never
  shrink-wraps to begin with) -- but unlike the earlier attempt, this
  fix no longer depends on any specific ancestor behavior to work.
